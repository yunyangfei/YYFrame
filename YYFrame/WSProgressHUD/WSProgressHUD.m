...
#pragma mark - Show with string (转圈圈➕文字)
...
+ (void)showSuccessWithStatus: (NSString *)string   //图标✅+文字
{
    [self showImage:WSProgressHUDSuccessDefaultImage() status:string];
}

+ (void)showErrorWithStatus: (NSString *)string     //图标❎+文字
{
...
#pragma mark - Shimmering String    (闪闪发光的)
...
{
    NSAssert([NSThread isMainThread], @"WSProgressHUD show Must on main thread");
    objc_setAssociatedObject(self, @selector(maskType), @(WSProgressHUDMaskTypeClear), OBJC_ASSOCIATION_ASSIGN);
    objc_setAssociatedObject(self, @selector(showImage), @(1), OBJC_ASSOCIATION_ASSIGN);
    objc_setAssociatedObject(self, @selector(hudType), @(WSProgressHUDTypeImage), OBJC_ASSOCIATION_ASSIGN);
    objc_setAssociatedObject(self, @selector(withoutType), @(WSProgressHUDMaskWithoutDefault), OBJC_ASSOCIATION_ASSIGN);
